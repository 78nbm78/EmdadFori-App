...
import { UpdateBrandAPI } from "@/services/Brands";

interface IProps {
  accessToken: string;
  brand: IBrandType | undefined;
  pageSlug: string;
}

const UpdateBrandForm = ({ accessToken, brand, pageSlug }: IProps) => {
  ...
  const onSubmit = async (data: IBrandType) => {
    try {
      ...
      const response: IBrandResponse | undefined = await UpdateBrandAPI({
        accessToken,
        pageSlug,
        data,
      });
