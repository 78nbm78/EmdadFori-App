...
import { IAddBlogResponse, IBlog, IBlogType } from "@/interfaces/Blog";
...
import { AddBlogAPI } from "../_core/requests";
...
const AddBlogForm = ({ accessToken }: IProps) => {
  ...
  const onSubmit = async (data: IBlogType) => {
    try {
      ...
      const response: IAddBlogResponse | undefined = await AddBlogAPI({
        accessToken,
        data,
      });
