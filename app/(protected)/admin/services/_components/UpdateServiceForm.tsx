...
interface IProps {
  accessToken: string;
  service: IServiceType | undefined;
  pageSlug: string;
}

const UpdateServiceForm = ({ accessToken, service, pageSlug }: IProps) => {
  const router = useRouter();

  const form = useForm<IServiceType>({
    resolver: zodResolver(IService),
    defaultValues: {
      title: service?.title || "",
      slug: service?.slug || "",
      googleTitle: service?.googleTitle || "",
  ...
  const onSubmit = async (data: IServiceType) => {
    try {
      data.slug = data.slug.split(" ").join("-").toLowerCase();

      const response: IServiceResponse | undefined = await UpdateServiceAPI({
        accessToken,
        pageSlug,
        data,
      });
